...
suite(`exact() works with value getter ability`, async ({ puppeteer: { page } }) => {
  ...
  const disabledValue = await page.evaluate(async () => {
          const ability = (window as unknown as WithGlobals).testState.eligiblePicking.exact(9),
                picks = [...(window as unknown as WithGlobals).testState.pickable.current.picks]
  ...
  await page.evaluate(() => (window as unknown as WithGlobals).testState.pickable.current.omit())
})
...
suite(`exact() works with reactive value getter ability`, async ({ puppeteer: { page } }) => {
  ...
  const enabledValue = await page.evaluate(async () => {
          (window as unknown as WithGlobals).testState.setAbilities(new Array(10).fill('enabled'))
  ...
  await page.evaluate(() => (window as unknown as WithGlobals).testState.pickable.current.omit())
  await page.evaluate(() => (window as unknown as WithGlobals).testState.setAbilities(new Array(10).fill('disabled')))
})
...
suite(`picks picked element's new location when elements are reordered`, async ({ puppeteer: { page } }) => {
  ...
  await page.evaluate(() => (window as unknown as WithGlobals).testState.setAbilities(new Array(10).fill('enabled')))

  const value = await page.evaluate(async () => {
          (window as unknown as WithGlobals).testState.pickable.current.pick(0);
          (window as unknown as WithGlobals).testState.reorder()
          await (window as unknown as WithGlobals).nextTick()
          return [...(window as unknown as WithGlobals).testState.pickable.current.picks]
        }),
        expected = [9]
...
suite(`omits when elements are removed and location is beyond the new end`, async ({ puppeteer: { page } }) => {
  ...
  const value = await page.evaluate(async () => {
          (window as unknown as WithGlobals).testState.pickable.current.pick(9);
...
suite(`omits disabled when reactive value getter watch source changes`, async ({ puppeteer: { page } }) => {
  ...
  const value = await page.evaluate(async () => {
          (window as unknown as WithGlobals).testState.pickable.current.pick(new Array(10).fill(0).map((_, index) => index));  
          (window as unknown as WithGlobals).testState.setAbilities(new Array(10).fill(0).map((_, index) => index % 2 === 0 ? 'enabled' : 'disabled'))
          await (window as unknown as WithGlobals).nextTick()
          return [...(window as unknown as WithGlobals).testState.pickable.current.picks]
        }),
        expected = [0, 2, 4, 6, 8]
